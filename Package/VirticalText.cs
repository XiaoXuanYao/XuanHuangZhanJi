...
public class VirticalText : BaseMeshEffect
{
    ...
    [Tooltip("一列最多想要容纳的字符数量<=0代表不自动换列")]
    ...
    private readonly List<string> _subStrList = new List<string>();
    public void SetText(string text)
    {
        ...
        _subStrList.Clear();
        int len = text.Length;
        int totleInsertCount = len / this._lineCount;
        if (len % this._lineCount != 0)
            totleInsertCount += 1;

        for (int i = 0; i < totleInsertCount; ++i)
        {
            int startIndex = i * this._lineCount;
            int totleLen = startIndex + this._lineCount;
            string sub;
            if (totleLen > len)
                sub = text.Substring(startIndex);
            else
                sub = text.Substring(startIndex, this._lineCount);
            if (sub.Length > 0)
                this._subStrList.Add(sub);
        }

        string finallyStr = string.Join("\n", this._subStrList);
        this.gameObject.GetComponent<Text>().text = finallyStr;
    }
